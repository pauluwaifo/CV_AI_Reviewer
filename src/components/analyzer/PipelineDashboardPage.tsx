...
import { getPersonalityFitTier } from "@/lib/personality-assessment";
import {
...
function ApplicationReviewCard({
  ...
  const personalityActionLabel = application.personalityAssessment
    ? "Edit assessment"
    : "Create assessment";
  ...
  return (
    <article className="space-y-6">
      <section className="overflow-hidden rounded-[4px] border border-[var(--workspace-form-border-soft)] bg-white p-6 shadow-[var(--workspace-form-shadow-md)] dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="space-y-6">
        <section className="rounded-[4px] border border-[var(--workspace-form-border-soft)] bg-white p-5 shadow-[var(--workspace-form-shadow-sm)] dark:border-gray-800 dark:bg-gray-900">
        ...
        <section className="rounded-[4px] border border-[var(--workspace-form-border-soft)] bg-white p-5 shadow-[var(--workspace-form-shadow-sm)] dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="mt-5 rounded-[4px] border border-[var(--workspace-form-border-soft)] bg-[var(--workspace-form-surface)] p-4 dark:border-gray-800 dark:bg-gray-950/60">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--workspace-form-accent-muted)] dark:text-gray-400">
              Recommendation
            </p>
          ...
          <div className="mt-5 rounded-xl border border-[var(--workspace-form-border-soft)] bg-[var(--workspace-form-surface)] p-4 dark:border-gray-800 dark:bg-gray-950/60">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-[var(--workspace-form-muted)] dark:text-gray-300">
                  {application.personalityAssessment
                    ? "Saved against this application and ready for recruiter review."
                    : "No saved assessment yet. Open the dedicated page to add one."}
                </p>
              </div>
              {application.personalityAssessment ? (
                <span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[var(--workspace-form-pill-text)] shadow-[0_1px_2px_rgba(15,23,42,0.08)] dark:bg-gray-900 dark:text-brand-200 dark:shadow-none">
                  {getPersonalityFitTier(application.personalityAssessment.fitScore)} fit
                </span>
              ) : null}
            </div>
            ...
            {application.personalityAssessment ? (
              <div className="mt-4 grid gap-2">
                <ReviewMetricCard
                  label="Bright"
                  value={`${application.personalityAssessment.brightAverage}%`}
                />
                <ReviewMetricCard
                  label="Derailer"
                  value={`${application.personalityAssessment.derailerAverage}%`}
                />
                <ReviewMetricCard
                  label="Values"
                  value={`${application.personalityAssessment.valuesAverage}%`}
                />
              </div>
            ) : null}
          ...
          <div className="mt-4 flex flex-wrap gap-3">
              <Link
                href={appendWorkspaceQuery(
                  `/personality?form=${encodeURIComponent(application.formId)}&application=${encodeURIComponent(application.id)}`,
                  settings.workspaceId
                )}
                className="inline-flex items-center justify-center rounded-lg bg-[var(--workspace-form-accent)] px-4 py-2.5 text-sm font-medium text-[var(--workspace-form-accent-text)] shadow-theme-xs transition hover:bg-[var(--workspace-form-accent-muted)]"
              >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            ...
            {application.personalityAssessment ? (
              <span className="rounded-full bg-[var(--workspace-form-surface)] px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.12em] text-[var(--workspace-form-muted)] dark:bg-gray-950/60 dark:text-gray-300">
                Personality {getPersonalityFitTier(application.personalityAssessment.fitScore)}
              </span>
            ) : null}
            {filterResult?.autoFiltered ? (
